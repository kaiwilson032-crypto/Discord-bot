...
async def on_message(message: discord.Message):
    ...
    realm_keywords = ["realm code", "realm-code", "realmcode", "realm", "code"]
    asking_keywords = ["where", "what", "how", "find", "get", "need", "looking for", "can i", "do you have"]
    
    is_asking = any(asking_kw in message_lower for asking_kw in asking_keywords)
    mentions_realm = any(realm_kw in message_lower for realm_kw in realm_keywords)
    
    if is_asking and mentions_realm:
        print(f"[REALM CODE] {message.author} asked about realm code in #{message.channel}")
...
async def set_realm_code(
    ...
    # Save to database
    RealmCodeDB.set_channel(interaction.guild.id, channel.id)
...
async def realm_code(interaction: discord.Interaction):
    ...
    try:
        ...
        embed = discord.Embed(
            title="❌ Error",
            description=f"Could not retrieve realm code location: {str(e)}",
            color=discord.Color.red()
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
