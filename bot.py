...
@app_commands.describe(
    action="Action: info, hatch, eggs, dragons, explore, train, battle, breed, reset, island"
)
async def dragon(interaction: discord.Interaction, action: str = "info"):
    ...
    if not DRAGON_SYSTEM_AVAILABLE:
        embed = discord.Embed(
            title="❌ Dragon System Unavailable",
            description="The dragon system module is not loaded",
            color=discord.Color.red()
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    try:
        ...
        if action == "info":
            ...
            embed.add_field(
                ...
                value="`/dragon hatch` - Get a dragon egg (5 min hatch, first hatches instantly!)\n`/dragon eggs` - View your eggs",
                inline=False
            )
            
            embed.add_field(
                name="🐲 Main Commands",
                value="`/dragon dragons` - View all your dragons with stats & abilities\n`/dragon island` - View your resources\n`/dragon reset` - Reset all progress",
                inline=False
            )
            
            embed.add_field(
                name="⚔️ Action Commands",
                value="`/dragon explore` - Find training items (4 min cooldown)\n`/dragon train` - Train using items to boost stats (1 hr cooldown)\n`/dragon battle` - Turn-based PvE battle until someone loses",
                inline=False
            )
            
            embed.add_field(
                name="👥 PvP & Breeding",
                value="`/duel @user` - Challenge another player's dragon\n`/dragon breed` - Breed 2 dragons for offspring",
                inline=False
            )
            
            embed.add_field(
                name="📚 Information",
                value="`/abilities` - View all dragon unique abilities\n`/inventory` - View training items & resources",
                inline=False
            )
            ...
            if not dragons:
                ...
                for i, dragon in enumerate(dragons[:10], 1):
                    ...
                    ability_info = DragonSystem.DRAGON_ABILITIES.get(species, {})
...
**Bond:** {'⭐' * (bond + 1)} (Lvl {bond})
**Ability:** {ability_name} (Lvl {ability})

**Stats:**
⚔️ Attack: {attack} | 🛡️ Defense: {defense}
...
"""
            ...
            class BattleView(discord.ui.View):
                def __init__(self, player_dragon, enemy_data):
                    super().__init__()
                    self.player_dragon = player_dragon
                    self.enemy_data = enemy_data
                    self.player_hp = DragonSystem.calculate_dragon_hp(player_dragon[4], player_dragon[7], player_dragon[3])
                    self.enemy_hp = enemy_data["hp"]
                    self.battle_log = []
                    self.player_turn = True
                    
                    button = discord.ui.Button(
                        label=f"Use {DragonSystem.DRAGON_ABILITIES.get(player_dragon[2], {}).get('name', 'Attack')}",
                        style=discord.ButtonStyle.danger,
                        custom_id="attack"
                    )
                    button.callback = self.attack
                    self.add_item(button)
                
                async def attack(self, inter: discord.Interaction):
                    if not self.player_turn:
                        await inter.response.defer()
                        return
                    
                    # Player attacks
                    ability_name = DragonSystem.DRAGON_ABILITIES.get(self.player_dragon[2], {}).get('name', 'Attack')
                    damage = DragonSystem.calculate_ability_damage(
                        self.player_dragon[2],
                        ability_name,
                        self.player_dragon[6],
                        self.player_dragon[4],
                        self.player_dragon[8],
                        self.player_dragon[9]
                    )
                    
                    self.enemy_hp -= damage
                    self.battle_log.append(f"🔥 Your **{self.player_dragon[2]}** used **{ability_name}** for **{damage}** damage!")
                    ...
                    enemy_ability = DragonSystem.DRAGON_ABILITIES.get(self.enemy_data["species"], {}).get('name', 'Attack')
                    enemy_damage = DragonSystem.calculate_ability_damage(
                        self.enemy_data["species"],
                        enemy_ability,
                        self.enemy_data["attack"],
                        self.enemy_data["level"],
                        self.enemy_data["speed"],
                        self.enemy_data["intelligence"]
                    )
                    
                    self.player_hp -= enemy_damage
                    self.battle_log.append(f"⚡ Enemy **{self.enemy_data['species']}** used **{enemy_ability}** for **{enemy_damage}** damage!")
                    ...
                    # Update embed
                    embed = discord.Embed(
                        title="⚔️ Dragon Battle",
                        color=discord.Color.orange()
                    )
                    embed.add_field(
                        name=f"Your {self.player_dragon[2]}",
                        value=f"❤️ HP: {max(0, self.player_hp)}/{DragonSystem.calculate_dragon_hp(self.player_dragon[4], self.player_dragon[7], self.player_dragon[3])}",
                        inline=True
                    )
                    embed.add_field(
                        name=f"Enemy {self.enemy_data['species']}",
                        value=f"❤️ HP: {max(0, self.enemy_hp)}/{self.enemy_data['hp']}",
                        inline=True
                    )
            ...
            enemy_data = {
                ...
                "hp": DragonSystem.calculate_dragon_hp(enemy_level, enemy_base["defense"] + (enemy_level * 2), "Common")
            }
            
            player_dragon = dragons[0]
            player_hp = DragonSystem.calculate_dragon_hp(player_dragon[4], player_dragon[7], player_dragon[3])
            
            embed = discord.Embed(
                title="⚔️ Dragon Battle Started!",
                color=discord.Color.orange()
            )
            embed.add_field(
                name=f"Your {player_dragon[2]}",
                value=f"❤️ HP: {player_hp}\nLvl: {player_dragon[4]}",
                inline=True
            )
            embed.add_field(
                name=f"Enemy {enemy_species}",
                value=f"❤️ HP: {enemy_data['hp']}\nLvl: {enemy_level}",
                inline=True
            )
            embed.add_field(name="Battle", value="Click the button to attack!", inline=False)
            
            view = BattleView(player_dragon, enemy_data)
            ...
            final_embed = discord.Embed(
                ...
                description=result,
                color=discord.Color.green() if view.player_hp > 0 else discord.Color.red()
            )
            final_embed.add_field(
                name="Your Dragon",
                value=f"Final HP: {max(0, view.player_hp)}",
                inline=True
            )
            final_embed.add_field(
                name="Enemy Dragon",
                value=f"Final HP: {max(0, view.enemy_hp)}",
                inline=True
            )
            ...
            class ResetView(discord.ui.View):
                ...
                async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
                    ...
                    embed = discord.Embed(
                        ...
                        description="Your dragon progress has been reset!",
                        color=discord.Color.green()
                    )
                    await interaction.response.send_message(embed=embed, ephemeral=True)
...
@bot.tree.command(name="duel", description="Challenge another player to a dragon battle")
@app_commands.describe(
    opponent="The player you want to duel"
)
async def duel(interaction: discord.Interaction, opponent: discord.User):
    """Challenge another player's dragon to battle!"""
    ...
    try:
        ...
        if challenger_id == opponent_id:
            embed = discord.Embed(
                title="❌ Can't Duel Yourself",
                description="You can't battle your own dragon!",
                color=discord.Color.red()
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        ...
        if not challenger_dragons:
            embed = discord.Embed(
                title="❌ You Have No Dragons",
                description="You need at least 1 dragon to duel!",
                color=discord.Color.red()
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        ...
        if not opponent_dragons:
            embed = discord.Embed(
                title="❌ Opponent Has No Dragons",
                description=f"{opponent.mention} doesn't have any dragons yet!",
                color=discord.Color.red()
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        
        # Step 1: Challenger selects dragon
        class DragonSelectView(discord.ui.View):
            def __init__(self, dragons):
                super().__init__()
                self.selected_dragon = None
                
                for i, dragon in enumerate(dragons[:5]):
                    dragon_id, name, species, rarity, level, xp, attack, defense, speed, intelligence, bond, ability = dragon
                    button = discord.ui.Button(
                        label=f"{species} (Lvl {level})",
                        style=discord.ButtonStyle.primary,
                        custom_id=f"dragon_{i}"
                    )
                    button.callback = self.select_dragon
                    self.add_item(button)
            
            async def select_dragon(self, inter: discord.Interaction):
                idx = int(inter.custom_id.split("_")[1])
                self.selected_dragon = self.dragons[idx]
                await inter.response.defer()
            
            @property
            def dragons(self):
                return getattr(self, '_dragons', [])
            
            @dragons.setter
            def dragons(self, value):
                self._dragons = value
        
        challenger_view = DragonSelectView(challenger_dragons)
        challenger_view.dragons = challenger_dragons
        
        select_embed = discord.Embed(
            title="⚔️ Select Your Battle Dragon",
            description=f"Choose which dragon you want to battle {opponent.mention} with:",
            color=discord.Color.gold()
        )
        
        await interaction.response.send_message(embed=select_embed, view=challenger_view, ephemeral=True)
        await asyncio.sleep(1)
        
        if not challenger_view.selected_dragon:
            return
        
        challenger_dragon = challenger_view.selected_dragon
        
        # Step 2: Opponent selects dragon
        opponent_view = DragonSelectView(opponent_dragons)
        opponent_view.dragons = opponent_dragons
        
        opponent_embed = discord.Embed(
            title="⚔️ Duel Challenge!",
            description=f"{interaction.user.mention} challenged you to a dragon duel!",
            color=discord.Color.gold()
        )
        opponent_embed.add_field(
            name="Challenger's Dragon",
            value=f"**{challenger_dragon[2]}** (Lvl {challenger_dragon[4]})",
            inline=False
        )
        opponent_embed.add_field(
            name="Choose Your Dragon:",
            value="Click a button below to accept the duel!",
            inline=False
        )
        
        try:
            await opponent.send(embed=opponent_embed, view=opponent_view)
            await interaction.followup.send("✅ Challenge sent! Waiting for opponent to select their dragon...", ephemeral=True)
        except discord.Forbidden:
            error_embed = discord.Embed(
                title="❌ DM Failed",
                description=f"Couldn't send DM to {opponent.mention}. They may have DMs disabled.",
                color=discord.Color.red()
            )
        ...
        if not opponent_view.selected_dragon:
            return
        
        opponent_dragon = opponent_view.selected_dragon
        
        # Step 3: Both players select abilities
        class AbilitySelectView(discord.ui.View):
            def __init__(self, dragon_species):
                super().__init__()
                self.selected_ability = None
                self.dragon_species = dragon_species
                
                ability = DragonSystem.DRAGON_ABILITIES.get(dragon_species, {})
                if ability:
                    button = discord.ui.Button(
                        label=ability.get("name", "Unknown Ability"),
                        style=discord.ButtonStyle.danger,
                        custom_id="ability_select"
                    )
                    button.callback = self.select_ability
                    self.add_item(button)
            
            async def select_ability(self, inter: discord.Interaction):
                self.selected_ability = DragonSystem.DRAGON_ABILITIES.get(self.dragon_species, {}).get("name", "Unknown")
                await inter.response.defer()
        
        # Challenger selects ability
        challenger_ability_view = AbilitySelectView(challenger_dragon[2])
        challenger_ability_embed = discord.Embed(
            title="🔥 Select Ability",
            description=f"Choose your **{challenger_dragon[2]}**'s ability:",
            color=discord.Color.orange()
        )
        ability_info = DragonSystem.DRAGON_ABILITIES.get(challenger_dragon[2], {})
        challenger_ability_embed.add_field(
            name=ability_info.get("name", "Unknown"),
            value=f"{ability_info.get('description', '')}\nPower: {ability_info.get('power', 0)} | Accuracy: {ability_info.get('accuracy', 0)}%",
            inline=False
        )
        
        await interaction.user.send(embed=challenger_ability_embed, view=challenger_ability_view)
        await asyncio.sleep(1)
        
        # Opponent selects ability
        opponent_ability_view = AbilitySelectView(opponent_dragon[2])
        opponent_ability_embed = discord.Embed(
            title="🔥 Select Ability",
            description=f"Choose your **{opponent_dragon[2]}**'s ability:",
            color=discord.Color.orange()
        )
        ability_info = DragonSystem.DRAGON_ABILITIES.get(opponent_dragon[2], {})
        opponent_ability_embed.add_field(
            name=ability_info.get("name", "Unknown"),
            value=f"{ability_info.get('description', '')}\nPower: {ability_info.get('power', 0)} | Accuracy: {ability_info.get('accuracy', 0)}%",
            inline=False
        )
        
        await opponent.send(embed=opponent_ability_embed, view=opponent_ability_view)
        await asyncio.sleep(1)
        
        if not challenger_ability_view.selected_ability or not opponent_ability_view.selected_ability:
            return
        
        # Calculate damage for both dragons
        challenger_damage = DragonSystem.calculate_ability_damage(
            challenger_dragon[2],
            challenger_ability_view.selected_ability,
            challenger_dragon[6],  # attack
            challenger_dragon[4],  # level
            challenger_dragon[8],  # speed
            challenger_dragon[9]   # intelligence
        )
        
        opponent_damage = DragonSystem.calculate_ability_damage(
            opponent_dragon[2],
            opponent_ability_view.selected_ability,
            opponent_dragon[6],  # attack
            opponent_dragon[4],  # level
            opponent_dragon[8],  # speed
            opponent_dragon[9]   # intelligence
        )
        
        challenger_wins = challenger_damage > opponent_damage
        
        # Award rewards
        if challenger_wins:
            coins = random.randint(100, 300)
            xp = random.randint(50, 150)
            result_text = f"🏆 You won! +{xp} XP, +{coins} Coins"
        else:
            coins = random.randint(30, 100)
            xp = random.randint(20, 50)
            result_text = f"😢 You lost! +{xp} XP, +{coins} Coins (consolation)"
        ...
        cursor.execute("""
            UPDATE dragons SET xp = xp + ? WHERE dragon_id = ?
        """, (xp, challenger_dragon[0]))
        cursor.execute("""
            UPDATE player_dragons SET coins = coins + ? WHERE user_id = ?
        """, (coins, challenger_id))
        
        # Same for opponent
        if opponent_wins := not challenger_wins:
            opp_coins = random.randint(100, 300)
            opp_xp = random.randint(50, 150)
            opp_result = f"🏆 You won! +{opp_xp} XP, +{opp_coins} Coins"
        else:
            opp_coins = random.randint(30, 100)
            opp_xp = random.randint(20, 50)
            opp_result = f"😢 You lost! +{opp_xp} XP, +{opp_coins} Coins (consolation)"
        
        cursor.execute("""
            UPDATE dragons SET xp = xp + ? WHERE dragon_id = ?
        """, (opp_xp, opponent_dragon[0]))
        cursor.execute("""
            UPDATE player_dragons SET coins = coins + ? WHERE user_id = ?
        """, (opp_coins, opponent_id))
        ...
        battle_embed.add_field(
            name=f"{interaction.user.name}'s {challenger_dragon[2]}",
            value=f"Ability: **{challenger_ability_view.selected_ability}**\nDamage: **{challenger_damage}**\nLevel: {challenger_dragon[4]}",
            inline=True
        )
        
        battle_embed.add_field(
            name=f"{opponent.name}'s {opponent_dragon[2]}",
            value=f"Ability: **{opponent_ability_view.selected_ability}**\nDamage: **{opponent_damage}**\nLevel: {opponent_dragon[4]}",
            inline=True
        )
        ...
        if challenger_wins:
            battle_embed.add_field(
                name="🏆 Winner",
                value=f"{interaction.user.mention} wins!",
                inline=False
            )
        else:
            battle_embed.add_field(
                name="🏆 Winner",
                value=f"{opponent.mention} wins!",
                inline=False
            )
        
        await interaction.channel.send(embed=battle_embed)
        await opponent.send(embed=battle_embed)
        
        # Send individual results
        challenger_result_embed = discord.Embed(
            title="Battle Result",
            description=result_text,
            color=discord.Color.green() if challenger_wins else discord.Color.red()
        )
        await interaction.user.send(embed=challenger_result_embed)
        
        opponent_result_embed = discord.Embed(
            title="Battle Result",
            description=opp_result,
            color=discord.Color.green() if not challenger_wins else discord.Color.red()
        )
        await opponent.send(embed=opponent_result_embed)
    
    except Exception as e:
        print(f"[DUEL] Error: {str(e)}")
        embed = discord.Embed(
            title="❌ Error",
            description=f"An error occurred: {str(e)}",
            color=discord.Color.red()
...
@bot.tree.command(name="inventory", description="View your dragon training items and resources")
async def inventory(interaction: discord.Interaction):
    """View your inventory of training items and resources"""
    ...
    try:
        ...
        embed = discord.Embed(
            ...
            description="Training items and resources",
            color=discord.Color.blue()
        )
        ...
        if inv['training_items']:
            ...
            embed.add_field(name="🎁 Training Items", value="No training items yet!\nFind them while exploring.", inline=False)
        
        embed.set_footer(text="Training items are used to train your dragons and level up their abilities!")
...
@bot.tree.command(name="abilities", description="View all dragon abilities")
async def abilities(interaction: discord.Interaction):
    """View unique abilities for all dragons"""
    ...
    try:
        ...
        for species, ability_info in list(DragonSystem.DRAGON_ABILITIES.items())[:10]:
            embed.add_field(
                name=f"🔥 {species}",
        ...
        embed.set_footer(text="Use /dragon train to level up your dragon's abilities!")
        
        await interaction.response.send_message(embed=embed, ephemeral=True)
    
    except Exception as e:
        print(f"[ABILITIES] Error: {str(e)}")
        embed = discord.Embed(
            title="❌ Error",
            description=f"An error occurred: {str(e)}",
            color=discord.Color.red()
...
# Initialize dragon tables on bot startup
if DRAGON_SYSTEM_AVAILABLE:
    DragonSystem.init_tables()
    print("[DRAGON] ✅ Dragon tables initialized")
