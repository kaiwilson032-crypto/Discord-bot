...
# Modal for handling application questions
class ApplicationModal(discord.ui.Modal):
    """Modal for displaying and collecting application answers"""
    
    def __init__(self, user_id: int, guild_id: int, questions: List[str], question_indices: List[int]):
        ...
        progress = question_indices[0] if question_indices else 0
        title = f"Application - Question {progress + 1}/{len(questions)}"
        super().__init__(title=title[:45])  # Discord modal title limit
        
        # Add form fields for each question in this batch
        for idx, q_idx in enumerate(question_indices):
            if q_idx < len(questions):
                question = questions[q_idx]
                # Truncate long questions for label
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        for answer in self.children:
            ApplicationDB.add_answer(self.user_id, self.guild_id, answer.value)
        ...
        if progress >= len(self.questions):
            ...
            await submit_application(interaction.user, interaction.guild, self.questions)
            ...
            # Show next question(s)
            remaining = len(self.questions) - progress
            # Show up to 5 questions per modal
            next_batch = min(5, remaining)
            question_indices = list(range(progress, progress + next_batch))
            
            modal = ApplicationModal(self.user_id, self.guild_id, self.questions, question_indices)
...
async def submit_application(user: discord.User, guild: discord.Guild, questions: List[str]):
    ...
    # Try to get the configured target channel first
    target_channel = None
    if target_channel_id:
        try:
            target_channel = await bot.fetch_channel(target_channel_id)
        except (discord.NotFound, discord.Forbidden, discord.HTTPException):
            # Channel not found or bot doesn't have access, will try fallback
            target_channel = None
    
    # Fallback: look for "applications" channel
    ...
    # Last resort: use first text channel
    if not target_channel:
        if guild.text_channels:
            target_channel = guild.text_channels[0]
    
    if not target_channel:
        print(f"⚠️ No target channel found for guild {guild.id}")
        return
    ...
    try:
        ...
        print(f"✅ Application from {user} submitted to {target_channel.mention}")
    except Exception as e:
...
async def setup_admin(
    ...
    # Collect non-empty questions (maintain all 10 if provided)
...
async def application(interaction: discord.Interaction):
    """Start the application process with a modal"""
    ...
    # Show first question(s) in a modal
    # Show up to 5 questions per modal
    batch_size = min(5, len(questions))
    question_indices = list(range(0, batch_size))
    
    modal = ApplicationModal(interaction.user_id, interaction.guild.id, questions, question_indices)
    await interaction.response.send_modal(modal)
