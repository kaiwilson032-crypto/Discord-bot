...
class GiveawayDB:
    ...
    def get_all_active() -> List[dict]:
        """Get all active (non-ended) giveaways"""
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
...
def parse_duration(duration_str: str) -> Optional[int]:
    ...
    Accepts e.g. '1 day', 'one day', '2 hours', 'thirty minutes', '1 hour', etc.
    ...
    # Match: optional number (digit or word), then unit
    pattern = re.compile(
        r'(\d+|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|'
        r'thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|'
        r'thirty|forty|fifty|sixty|seventy|eighty|ninety|a|an)?\s*'
    ...
    if raw_num is None:
        ...
        amount = word_to_num.get(raw_num.lower(), 1)
    if unit in ('day', 'days'):
        return amount * 1440
...
@bot.tree.command(name="giveaway-begin", description="Start a giveaway (Admin Only)")
@app_commands.describe(
...
async def giveaway_start(
    ...
    """Start a new giveaway with a natural language duration"""
    ...
    try:
        ...
        if not total_minutes or total_minutes <= 0:
            embed = discord.Embed(
                ...
                description="Couldn't understand that duration. Try something like `1 day`, `two hours`, or `30 minutes`.",
                color=discord.Color.red()
            )
            await interaction.followup.send(embed=embed, ephemeral=True)
            return
        ...
        # Build a clean display string from total_minutes
        display_days = total_minutes // 1440
        display_hours = (total_minutes % 1440) // 60
        display_mins = total_minutes % 60
        duration_parts = []
        if display_days:
            duration_parts.append(f"{display_days} day{'s' if display_days != 1 else ''}")
        if display_hours:
            duration_parts.append(f"{display_hours} hour{'s' if display_hours != 1 else ''}")
        if display_mins:
            duration_parts.append(f"{display_mins} minute{'s' if display_mins != 1 else ''}")
        ...
        embed.add_field(name="Ends in", value=duration_text, inline=True)
        ...
        # Save to database (store total days as the duration unit for backward compatibility)
        # Convert minutes to approximate days for storage
        duration_days = total_minutes / 1440
        GiveawayDB.create(giveaway_id, interaction.guild.id, channel_id, giveaway_message.id, prize, duration_days, winners, interaction.user.id, end_time)
        ...
        print(f"[GIVEAWAY] {interaction.user} started giveaway: {prize} (Duration: {duration_text})")
    except Exception as e:
        print(f"[GIVEAWAY ERROR] {str(e)}")
        try:
            await interaction.followup.send(f"❌ Error: {str(e)}", ephemeral=True)
...
async def check_giveaways():
    """Background task to check and auto-end giveaways when time expires"""
    try:
        ...
        for giveaway in giveaways:
            ...
            # Parse end_at if it's a string
            if isinstance(end_at, str):
                end_at = datetime.fromisoformat(end_at)
            
            # Check if giveaway time has expired
            if current_time >= end_at:
                try:
                    ...
                    if entries:
                        ...
                        if guild:
                            ...
                            if channel:
                                ...
                                # Send announcement
                                await channel.send(embed=embed)
                                print(f"[GIVEAWAY] ✅ Auto-ended giveaway {giveaway_id}, winners announced")
                    else:
                        # No entries - still end the giveaway
                        guild = bot.get_guild(giveaway['guild_id'])
                        if guild:
                            channel = guild.get_channel(giveaway['channel_id'])
                            if channel:
                    ...
                    print(f"[GIVEAWAY ERROR] Failed to auto-end giveaway {giveaway_id}: {str(e)}")
    except Exception as e:
        print(f"[GIVEAWAY BACKGROUND TASK ERROR] {str(e)}")
