...
class ApplicationModal(discord.ui.Modal):
    ...
    def __init__(self, user_id: int, guild_id: int, all_questions: List[str], 
        ...
        for idx in question_indices:
            ...
            # Create placeholder text showing question number
            placeholder = f"Q{idx + 1}: {question[:50]}..." if len(question) > 50 else f"Q{idx + 1}: {question}"
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if next_batch_index < self.total_batches:
            ...
            embed = discord.Embed(
                ...
                description=f"Answers saved! Proceeding to batch {next_batch_index + 1}...",
...
async def send_application_to_channel(guild_id: int, user: discord.User, questions: List[str], answers: List[str]):
    ...
    # Find the channel
    bot_instance = discord.utils.find(lambda b: b.id, [bot])
