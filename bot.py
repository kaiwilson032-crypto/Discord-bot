...
async def inactive_keeper_task():
    """Send a dot to all inactive keeper channels every 5 minutes"""
    try:
        ...
        for guild_id, channel_id in active_keepers:
            try:
                guild = bot.get_guild(guild_id)
                if not guild:
...
async def inactive_setup(
    ...
    try:
        ...
        embed = discord.Embed(
            ...
            description=f"Bot will now post a '.' in {channel.mention} every 5 minutes to stay active",
            color=discord.Color.green()
        )
        embed.add_field(name="Channel", value=channel.mention, inline=True)
        embed.add_field(name="Interval", value="Every 5 minutes", inline=True)
