...
async def on_message(message: discord.Message):
    ...
    try:
        ...
        # Check if any pattern matches (word boundary aware)
        is_asking_about_ticket = any(pattern in message_lower for pattern in ticket_asking_patterns)
        
        print(f"[TICKET SCAN] #{message.channel} | {message.author}: '{message.content[:60]}' | Asking: {is_asking_about_ticket}")
        
        if is_asking_about_ticket:
            print(f"[TICKET TRIGGER] ✅ {message.author} asked about creating a ticket in #{message.channel}")
            print(f"[TICKET TRIGGER] Message content: '{message.content}'")
