...
async def on_message(message: discord.Message):
    ...
    try:
        ...
        if message.mentions and len(message.mentions) > 0 and message.mentions[0] == bot.user:
            # Remove all mentions from content to get the actual command
            content_after_mentions = message.content
            for mention in message.mentions:
                content_after_mentions = content_after_mentions.replace(mention.mention, "").strip()
            
            # If user just mentioned the bot with no command, show help
            if not content_after_mentions:
                embed = discord.Embed(
                    title="🐉 Dragon Rider System",
                    description="Use `@bot dragon info` for help with all commands!",
                    color=discord.Color.orange()
                )
                embed.add_field(
                    name="Quick Start",
                    value="`@bot dragon hatch` - Get your first dragon egg\n`@bot dragon dragons` - View your dragons\n`@bot dragon info` - Full command guide",
                    inline=False
                )
                await message.reply(embed=embed)
                return
            
            # Check if the command is dragon-related
            if content_after_mentions.startswith("dragon"):
                args = content_after_mentions.split()
                ...
                # Bot was mentioned but command wasn't dragon-related
                return
        else:
            # Bot wasn't mentioned, skip this handler
            return
        ...
        if action == "info":
                ...
                embed = discord.Embed(title="⚠️ Delete All Progress?", description="Reply with `yes` to confirm.", color=discord.Color.red())
                ...
                try:
                    ...
                    if response.content.lower() == "yes":
                        ...
                        confirm_embed = discord.Embed(title="✅ Reset Complete", color=discord.Color.green())
                        await response.reply(embed=confirm_embed)
                    else:
                        cancel_embed = discord.Embed(title="❌ Cancelled", color=discord.Color.red())
                        await response.reply(embed=cancel_embed)
                except asyncio.TimeoutError:
                    await msg.reply("Reset cancelled (timeout)")
