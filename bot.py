...
def init_db():
    ...
    # Table for inactive keeper settings
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS inactive_keepers (
            guild_id INTEGER PRIMARY KEY,
            channel_id INTEGER NOT NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
...
class InactiveKeeperDB:
    """Database helper class for inactive keeper system"""
    
    @staticmethod
    def set_channel(guild_id: int, channel_id: int):
        """Set the inactive keeper channel for a server"""
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO inactive_keepers (guild_id, channel_id, created_at, updated_at)
            VALUES (?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
            ON CONFLICT(guild_id) DO UPDATE SET
                channel_id = ?,
                updated_at = CURRENT_TIMESTAMP
    ...
    @staticmethod
    def get_channel(guild_id: int) -> Optional[int]:
        """Get the inactive keeper channel for a server"""
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("SELECT channel_id FROM inactive_keepers WHERE guild_id = ?", (guild_id,))
        result = cursor.fetchone()
        conn.close()
    ...
    @staticmethod
    def get_all_active() -> List[tuple]:
        """Get all configured inactive keeper channels (guild_id, channel_id)"""
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("SELECT guild_id, channel_id FROM inactive_keepers")
    ...
    @staticmethod
    def delete_channel(guild_id: int):
        """Delete the inactive keeper channel for a server"""
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("DELETE FROM inactive_keepers WHERE guild_id = ?", (guild_id,))
        conn.commit()
        conn.close()
...
async def on_message(message: discord.Message):
    ...
    try:
        ...
        # Simple keyword check - just look for both "realm" and "code" or "code" and "?" 
        is_asking_about_realm = ("realm" in message_lower and ("code" in message_lower or "?" in message_lower)) or \
                                ("code" in message_lower and "?" in message_lower and "realm" in message_lower)
        
        print(f"[REALM DETECT] Asking about realm: {is_asking_about_realm}")
        
        if is_asking_about_realm and message.guild:
            print(f"[REALM TRIGGER] {message.author} asked about realm in #{message.channel}")
            ...
            if channel_id:
                ...
                if channel:
                    ...
                    print(f"[REALM] ✅ Auto-replied to {message.author}")
    except Exception as e:
...
async def on_ready():
    ...
    # Start inactive keeper background task (but check if any are configured first)
    try:
        active_keepers = InactiveKeeperDB.get_all_active()
        if active_keepers:
            print(f"[INACTIVE] Found {len(active_keepers)} configured keeper(s), starting task...")
            if not inactive_keeper_task.is_running():
                inactive_keeper_task.start()
        else:
            print(f"[INACTIVE] No inactive keepers configured, task disabled")
    except Exception as e:
        print(f"[INACTIVE] Error checking keepers: {e}")
...
async def inactive_keeper_task():
    """Send a dot to all inactive keeper channels every 10 minutes"""
    # TEMPORARILY DISABLED TO FIX RATE LIMITING
    return
    ...
    try:
        ...
        if not active_keepers:
            print(f"[INACTIVE] No active keepers configured")
            return
        
        print(f"[INACTIVE] Processing {len(active_keepers)} active keeper channel(s)")
        
        for idx, (guild_id, channel_id) in enumerate(active_keepers):
            try:
                # Add delay between messages to avoid rate limiting
                if idx > 0:
                    await asyncio.sleep(2)  # 2 seconds between servers
                
                guild = bot.get_guild(guild_id)
                if not guild:
                    print(f"[INACTIVE] Guild {guild_id} not found, skipping")
                    continue
                
                channel = guild.get_channel(channel_id)
                if not channel:
                    print(f"[INACTIVE] Channel {channel_id} not found in {guild.name}, skipping")
                    continue
                
                # Send a silent dot (won't notify anyone due to no @mentions)
                try:
                    await channel.send(".")
                    print(f"[INACTIVE] ✅ Sent keep-alive ping to {guild.name} #{channel.name}")
                except discord.Forbidden:
                    print(f"[INACTIVE] ❌ No permission to send in {guild.name} #{channel.name}")
                except discord.HTTPException as e:
                    if e.status == 429:
                        print(f"[INACTIVE] Rate limited, backing off...")
                        await asyncio.sleep(5)
                    else:
                        print(f"[INACTIVE] ❌ Error sending to {guild.name}: {str(e)}")
            except Exception as e:
                print(f"[INACTIVE] Error processing guild {guild_id}: {str(e)}")
    except Exception as e:
        print(f"[INACTIVE TASK ERROR] {str(e)}")

@inactive_keeper_task.before_loop
async def before_inactive_keeper():
    """Wait until bot is ready before starting the task"""
    await bot.wait_until_ready()
    print(f"[INACTIVE] Task started, will ping every 10 minutes")
...
@bot.tree.command(name="inactive-setup", description="Set up a channel for the bot to stay active (Admin Only)")
@app_commands.describe(
    channel="The channel to post activity pings to (won't notify anyone)"
)
async def inactive_setup(
    interaction: discord.Interaction,
    channel: discord.TextChannel
):
    """Set up inactive keeper channel"""
    ...
    try:
        ...
        if not interaction.user.guild_permissions.administrator:
            embed = discord.Embed(
                title="❌ Permission Denied",
                description="Only server administrators can set up the inactive keeper.",
                color=discord.Color.red()
            )
            await interaction.followup.send(embed=embed, ephemeral=True)
            return
        ...
        if not channel.permissions_for(interaction.guild.me).send_messages:
            embed = discord.Embed(
                title="❌ No Permission",
                description=f"Bot cannot send messages in {channel.mention}",
                color=discord.Color.red()
            )
            await interaction.followup.send(embed=embed, ephemeral=True)
            return
        ...
        import asyncio
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, InactiveKeeperDB.set_channel, interaction.guild.id, channel.id)
        
        embed = discord.Embed(
            title="✅ Inactive Keeper Activated",
            description=f"Bot will now post a '.' in {channel.mention} every 10 minutes to stay active",
            color=discord.Color.green()
        )
        embed.add_field(name="Channel", value=channel.mention, inline=True)
        embed.add_field(name="Interval", value="Every 10 minutes", inline=True)
        
        await interaction.followup.send(embed=embed, ephemeral=True)
        print(f"[INACTIVE] Admin {interaction.user} set up inactive keeper in {channel.name}")
    except Exception as e:
        print(f"[INACTIVE ERROR] {str(e)}")
        try:
            await interaction.followup.send(f"❌ Error: {str(e)}", ephemeral=True)
        except:
            pass
...
@bot.tree.command(name="inactive-disable", description="Disable the inactive keeper (Admin Only)")
async def inactive_disable(interaction: discord.Interaction):
    """Disable inactive keeper"""
    ...
    try:
        ...
        if not interaction.user.guild_permissions.administrator:
            embed = discord.Embed(
                title="❌ Permission Denied",
                description="Only server administrators can disable the inactive keeper.",
                color=discord.Color.red()
            )
            await interaction.followup.send(embed=embed, ephemeral=True)
            return
        ...
        import asyncio
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, InactiveKeeperDB.delete_channel, interaction.guild.id)
        
        embed = discord.Embed(
            title="✅ Inactive Keeper Disabled",
            description="The bot will no longer post activity pings.",
            color=discord.Color.orange()
        )
        
        await interaction.followup.send(embed=embed, ephemeral=True)
        print(f"[INACTIVE] Admin {interaction.user} disabled inactive keeper")
    except Exception as e:
        print(f"[INACTIVE ERROR] {str(e)}")
        try:
            await interaction.followup.send(f"❌ Error: {str(e)}", ephemeral=True)
