...
async def on_message(message: discord.Message):
    ...
    # Check if someone is asking about realm code (ONLY in guild channels)
    if message.guild:
        ...
        print(f"[REALM CHECK] #{message.channel} | {message.author}: '{message.content[:50]}' | Realm: {has_realm_keyword} | Asking: {has_asking_keyword}")
            ...
            try:
                ...
                if channel_id:
                    ...
                    if channel:
                        ...
                        print(f"[REALM CODE] ✅ Replied with realm code location to {message.author}")
                    else:
                        print(f"[REALM CODE] Channel ID {channel_id} not found in guild")
                else:
                    print(f"[REALM CODE] No realm code configured for guild {message.guild.name}")
            except Exception as e:
                print(f"[REALM CODE ERROR] Failed to respond: {type(e).__name__}: {str(e)}")
