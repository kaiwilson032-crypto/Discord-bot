...
from training import setup_training_commands, init_training_db
...
class GiveawayDB:
    ...
    @staticmethod
    def create(giveaway_id: str, guild_id: int, channel_id: int, message_id: int, prize: str, duration_minutes: int, winners: int, host_id: int, end_at: datetime):
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO giveaways (giveaway_id, guild_id, channel_id, message_id, prize, days, winners, host_id, end_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (giveaway_id, guild_id, channel_id, message_id, prize, duration_minutes, winners, host_id, end_at))
        conn.commit()
        conn.close()
    ...
    @staticmethod
    def add_entry(giveaway_id: str, user_id: int) -> bool:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        try:
            cursor.execute("""
    ...
    def get_all_active() -> List[dict]:
        ...
        return [{
            ...
            'prize': r[4], 'winners': r[6], 'host_id': r[7],
            'created_at': r[8], 'end_at': r[9], 'ended': r[10]
        } for r in results]
...
async def on_message(message: discord.Message):
    ...
    # Wilson name detection
    try:
        import re as _re
        if _re.search(r'\bwill?s+on\b', message.content, _re.IGNORECASE):
            await message.channel.send(
                "Created by **Wilson**. Maintained by luck. Powered by caffeine, duct tape, and questionable decisions.\n\n"
                "Built from scratch by **Wilson**. If I break, that\'s his fault. If I work, that\'s his genius. If I somehow do both at the same time, that\'s considered a feature.\n\n"
                "Please keep expectations somewhere between \"calculator\" and \"sentient toaster.\""
            )
            return
    except Exception as e:
        print(f"[WILSON DETECT ERROR] {e}")
...
async def on_ready():
    ...
    init_training_db()          # Initialize training tables
    setup_training_commands(bot, firebase_db)  # Register training slash commands
    
    # Start giveaway background task
    if not check_giveaways.is_running():
        check_giveaways.start()
        print("✅ Giveaway countdown task started")
...
def _parse_duration(duration_str: str) -> Optional[int]:
    """Parse natural language duration into total minutes.
    Accepts: '1 day', 'one day', '2 hours', 'thirty minutes', 'an hour', '30 mins', etc.
    Returns total minutes or None if unparseable."""
    word_to_num = {
    ...
    pattern = re.compile(
        ...
        r'(days?|hours?|hrs?|minutes?|mins?)',
        re.IGNORECASE
    )
    match = pattern.search(s)
    if not match:
        return None
    raw_num, unit = match.group(1), match.group(2).lower()
    if raw_num is None:
        amount = 1
    elif raw_num.isdigit():
        amount = int(raw_num)
    else:
        amount = word_to_num.get(raw_num, 1)
...
def _format_duration(total_minutes: int) -> str:
    """Format total minutes into a readable string."""
    ...
    try:
        ...
        if not interaction.user.guild_permissions.administrator:
            await interaction.followup.send(embed=discord.Embed(
                title="❌ Permission Denied",
                description="Only admins can set up the giveaway channel.",
                color=discord.Color.red()
            ), ephemeral=True)
            return
...
def _build_giveaway_embed(prize: str, end_time: datetime, winners: int, host_id: int, giveaway_id: str, ended: bool = False) -> discord.Embed:
    """Build the giveaway embed with live countdown or ended state."""
    if ended:
        embed = discord.Embed(title="🎉 GIVEAWAY (ENDED) 🎉", description=f"**{prize}**", color=discord.Color.greyple())
        embed.add_field(name="⏱ Time Remaining", value="Giveaway has ended!", inline=True)
    else:
        now = datetime.utcnow()
    ...
    embed.set_footer(text=f"Ends at {end_time.strftime('%Y-%m-%d %H:%M')} UTC | ID: {giveaway_id}")
    return embed
...
@app_commands.describe(
    ...
    duration="How long? e.g. '1 day', 'two hours', '30 minutes'",
    ...
    try:
        ...
        if not total_minutes or total_minutes <= 0:
            await interaction.followup.send(embed=discord.Embed(
                ...
                description="Couldn't understand that duration.\nTry: `1 day`, `two hours`, `30 minutes`, `an hour`",
                color=discord.Color.red()
            ), ephemeral=True)
            return
        ...
        if not giveaway_channel:
            await interaction.followup.send(embed=discord.Embed(
                title="❌ Channel Not Found",
                description="The configured giveaway channel no longer exists. Use `/giveaway-setup` to set a new one.",
                color=discord.Color.red()
            ), ephemeral=True)
            return
        ...
        duration_text = _format_duration(total_minutes)

        embed = _build_giveaway_embed(prize, end_time, winners, interaction.user.id, giveaway_id)
        giveaway_message = await giveaway_channel.send(embed=embed)
        await giveaway_message.add_reaction("🎉")

        GiveawayDB.create(giveaway_id, interaction.guild.id, channel_id, giveaway_message.id, prize, total_minutes, winners, interaction.user.id, end_time)

        await interaction.followup.send(embed=discord.Embed(
            title="✅ Giveaway Started",
            description=f"Posted in {giveaway_channel.mention}\n**Duration:** {duration_text}\n**ID:** `{giveaway_id}`\nUse `/giveaway-end` with that ID to end early.",
            color=discord.Color.green()
        ), ephemeral=True)
        print(f"[GIVEAWAY] {interaction.user} started giveaway {giveaway_id}: {prize} ({duration_text})")
    except Exception as e:
        print(f"[GIVEAWAY ERROR] {e}")
        try:
            await interaction.followup.send(f"❌ Error: {e}", ephemeral=True)
...
@bot.tree.command(name="giveaway-end", description="End a giveaway early and pick winners (Admin Only)")
@app_commands.describe(giveaway_id="The giveaway ID shown in the footer of the giveaway message")
async def giveaway_end(interaction: discord.Interaction, giveaway_id: str):
    try:
        ...
        giveaway = GiveawayDB.get(giveaway_id.upper())
        if not giveaway:
            await interaction.followup.send(embed=discord.Embed(
                title="❌ Giveaway Not Found",
                description=f"No giveaway with ID `{giveaway_id.upper()}`. Check the footer of the giveaway message.",
                color=discord.Color.red()
            ), ephemeral=True)
            return
        ...
        gid = giveaway_id.upper()
        entries = GiveawayDB.get_entries(gid)
        GiveawayDB.end_giveaway(gid)

        channel = interaction.guild.get_channel(giveaway['channel_id'])

        # Update original message embed to show ended state
        if channel and giveaway['message_id']:
            try:
                end_at = giveaway['end_at']
                if isinstance(end_at, str):
                    end_at = datetime.fromisoformat(end_at)
                orig_msg = await channel.fetch_message(giveaway['message_id'])
                ended_embed = _build_giveaway_embed(giveaway['prize'], end_at, giveaway['winners'], giveaway['host_id'], gid, ended=True)
        ...
        if not entries:
            ...
            await interaction.followup.send(embed=discord.Embed(
                ...
                description="No entries — no winners.",
                color=discord.Color.orange()
            ), ephemeral=True)
            return
        ...
        result_embed.add_field(name="🥇 Winners", value=winners_text, inline=False)
        result_embed.add_field(name="Total Entries", value=str(len(entries)), inline=True)

        if channel:
            await channel.send(embed=result_embed)
            # Plain text ping so winners actually get notified
            ping_text = "🎉 Congratulations " + " ".join([f"<@{w}>" for w in winner_ids]) + f"! You won **{giveaway['prize']}**!"
            await channel.send(ping_text)

        await interaction.followup.send(embed=discord.Embed(
            title="✅ Giveaway Ended",
            description=f"Winners: {', '.join([f'<@{w}>' for w in winner_ids])}",
            color=discord.Color.green()
        ), ephemeral=True)
...
async def check_giveaways():
    """Background task to auto-end giveaways when time expires"""
    try:
        giveaways = GiveawayDB.get_all_active()
        if not giveaways:
            return
        ...
        for giveaway in giveaways:
            ...
            if current_time < end_at:
                # Still running — update the countdown on the original embed
                try:
                    if channel and giveaway['message_id']:
                        orig_msg = await channel.fetch_message(giveaway['message_id'])
                        updated_embed = _build_giveaway_embed(
                            giveaway['prize'], end_at, giveaway['winners'],
                            giveaway['host_id'], giveaway_id, ended=False
                        )
                        await orig_msg.edit(embed=updated_embed)
                except Exception:
                    pass
                continue
            ...
            try:
                ...
                try:
                    if channel and giveaway['message_id']:
                        ...
                        ended_embed = _build_giveaway_embed(
                            giveaway['prize'], end_at, giveaway['winners'],
                            giveaway['host_id'], giveaway_id, ended=True
                        )
                        await orig_msg.edit(embed=ended_embed)
                except Exception:
                    pass
                ...
                if entries:
                    ...
                    if channel:
                        ...
                        ping_text = "🎉 Congratulations " + " ".join([f"<@{w}>" for w in winner_ids]) + f"! You won **{giveaway['prize']}**!"
                        await channel.send(ping_text)
                    print(f"[GIVEAWAY] ✅ Auto-ended {giveaway_id}, {num_winners} winner(s) announced")
                else:
                    result_embed = discord.Embed(
                        title="❌ Giveaway Ended — No Entries",
                        description=f"**Prize:** {giveaway['prize']}\nNo one entered.",
                    ...
                    if channel:
                        await channel.send(embed=result_embed)
                    print(f"[GIVEAWAY] Giveaway {giveaway_id} ended with no entries")

                GiveawayDB.end_giveaway(giveaway_id)
            except Exception as e:
                print(f"[GIVEAWAY ERROR] Failed to auto-end {giveaway_id}: {e}")
    except Exception as e:
        print(f"[GIVEAWAY BACKGROUND TASK ERROR] {e}")
...
async def on_reaction_add(reaction: discord.Reaction, user: discord.User):
    """Handle giveaway 🎉 reaction entries"""
    if user.bot or reaction.emoji != "🎉":
        return
    try:
        message = reaction.message
        if message.embeds and message.embeds[0].footer.text and "ID: " in message.embeds[0].footer.text:
            giveaway_id = message.embeds[0].footer.text.split("ID: ")[1].strip()
    ...
    try:
        ...
        if message.embeds and message.embeds[0].footer.text and "ID: " in message.embeds[0].footer.text:
            ...
            if giveaway and not giveaway['ended']:
                ...
                print(f"[GIVEAWAY] {user} left giveaway {giveaway_id}")
    except Exception as e:
        print(f"[GIVEAWAY REACTION ERROR] {e}")
