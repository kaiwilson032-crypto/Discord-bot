...
# -------------------------
# DRAGON RIDER SYSTEM - DATABASE
# -------------------------

class DragonDB:
    """Database for Dragon Rider game system"""
    ...
    @staticmethod
    def init_dragon_tables():
        """Initialize dragon-related tables"""
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS player_islands (
                ...
                hatchery_level INTEGER DEFAULT 1,
                training_grounds_level INTEGER DEFAULT 1,
                stable_level INTEGER DEFAULT 1,
    ...
    @staticmethod
    def hatch_egg(user_id: int, guild_id: int) -> dict:
        """Create a new egg for user"""
        import random
        import uuid
        ...
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO dragon_eggs (egg_id, user_id, guild_id, rarity)
            VALUES (?, ?, ?, ?)
        """, (egg_id, user_id, guild_id, rarity))
        conn.commit()
        conn.close()
        
        return {"egg_id": egg_id, "rarity": rarity}
    
    @staticmethod
    def open_egg(user_id: int, guild_id: int, egg_id: str) -> dict:
        ...
        # Get random species
        species = random.choice(DragonDB.DRAGON_SPECIES[rarity])
        
        # Generate stats with variation
        base_stats = DragonDB.DRAGON_STATS.get(species, {
...
# -------------------------
# DRAGON RIDER SYSTEM - COMMANDS
# -------------------------

@bot.tree.command(name="dragon", description="Dragon Rider game system")
@app_commands.describe(
    action="Action: info, hatch, eggs, dragons, island"
)
    ...
    try:
        ...
        if action == "info":
            ...
            embed.add_field(
                ...
                value="`/dragon hatch` - Get a random dragon egg\n`/dragon eggs` - View your eggs",
                inline=False
            )
            
            embed.add_field(
                name="🐲 Dragon Commands",
                value="`/dragon dragons` - View your dragons\n`/dragon info` - This guide\n`/dragon island` - View your island",
                inline=False
            )
            
            embed.add_field(
                name="💎 Egg Rarities",
                value="**Common** (60%) - Gronckle, Terrible Terror, Hideous Zippleback\n**Rare** (25%) - Deadly Nadder, Monstrous Nightmare, Rumblehorn\n**Epic** (10%) - Stormcutter, Skrill, Snow Wraith\n**Legendary** (4%) - Bewilderbeast, Red Death, Screaming Death\n**Alpha** (1%) - Titan Wing Night Fury, Alpha Bewilderbeast",
                inline=False
            )
            ...
            embed.add_field(
                name="🏆 Endgame Goals",
                value="Collect every dragon species • Discover rare hybrids • Max dragon stats • Reach Soul Bond • Obtain Alpha Dragons • Become a Dragon Master",
                inline=False
            )
            ...
            egg = DragonDB.hatch_egg(interaction.user.id, interaction.guild.id)
            
            embed = discord.Embed(
                title="🥚 You've received a dragon egg!",
                description=f"**Rarity:** {egg['rarity']}\n\nYour egg is waiting to be hatched!",
                color=discord.Color.gold()
            )
            embed.set_footer(text=f"Egg ID: {egg['egg_id'][:8]}...")
            ...
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.cursor()
            cursor.execute("SELECT egg_id, rarity FROM dragon_eggs WHERE user_id = ? ORDER BY created_at DESC", 
                          (interaction.user.id,))
            ...
            if not eggs:
                embed = discord.Embed(
                    title="🥚 No Eggs",
                    description="You don't have any dragon eggs. Use `/dragon hatch` to get one!",
                    color=discord.Color.blue()
                )
            else:
                embed = discord.Embed(
                    title="🥚 Your Dragon Eggs",
                    description=f"You have **{len(eggs)}** egg(s)",
                    color=discord.Color.blue()
                )
                
                for i, (egg_id, rarity) in enumerate(eggs[:10], 1):
                    embed.add_field(
                        name=f"Egg {i} - {rarity}",
                        value=f"ID: `{egg_id[:8]}...`",
                        inline=True
            ...
            cursor = conn.cursor()
            cursor.execute("""
                SELECT dragon_id, species, rarity, level, attack, defense, speed, intelligence 
                FROM dragons 
                WHERE user_id = ? 
            ...
            if not dragons:
                embed = discord.Embed(
                    ...
                    description="You don't have any dragons. Hatch an egg with `/dragon hatch`!",
                    color=discord.Color.blue()
                )
            else:
                embed = discord.Embed(
                    title="🐉 Your Dragons",
                    description=f"You own **{len(dragons)}** dragon(s)",
                    color=discord.Color.red()
                )
                
                for i, dragon in enumerate(dragons[:5], 1):
                    dragon_id, species, rarity, level, attack, defense, speed, intelligence = dragon
                    embed.add_field(
                        name=f"{i}. {species} - {rarity} (Lvl {level})",
                        value=f"⚔️ ATK: {attack} | 🛡️ DEF: {defense} | ⚡ SPD: {speed} | 🧠 INT: {intelligence}",
                        inline=False
                    )
            ...
            cursor = conn.cursor()
            cursor.execute("""
                SELECT coins, hatchery_level, training_grounds_level, stable_level, workshop_level
                FROM player_islands
                WHERE user_id = ?
            """, (interaction.user.id,))
            result = cursor.fetchone()
            ...
            if not result:
                ...
                cursor = conn.cursor()
                cursor.execute("""
                    INSERT INTO player_islands (user_id, guild_id)
                    VALUES (?, ?)
            ...
            coins, hatchery, training, stable, workshop = result
            ...
            embed.add_field(name="🥚 Hatchery", value=f"Level {hatchery}", inline=True)
            embed.add_field(name="🏋️ Training Grounds", value=f"Level {training}", inline=True)
            embed.add_field(name="🐲 Dragon Stable", value=f"Level {stable}", inline=True)
            embed.add_field(name="🔧 Workshop", value=f"Level {workshop}", inline=True)
            ...
            embed = discord.Embed(
                ...
                description="Use: `info`, `hatch`, `eggs`, `dragons`, or `island`",
                color=discord.Color.red()
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
...
# Initialize dragon tables on startup
DragonDB.init_dragon_tables()
